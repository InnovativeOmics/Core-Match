#!/usr/bin/env Rscript
# source(paste("","EIC_MS1_AIF_QRAI_fns.R",sep=""))
...
extract_AIF <- function( arguments ){
    # Convert an mzml file with multiple collision energies to ms2 with the fragments
    # output folder is really the InputDirectory for AIF
    arguments$fn_mzml_wpath = file.path(arguments$path_to_output_folder, arguments$fn_mzml)
    ...
    arguments$fn_FT_wpath = file.path(arguments$path_to_output_folder, arguments$fn_FT)
    print("_____________________________")
    # print(arguments$path_to_input_folder)
    # print(arguments$fn_mzml)
    print(arguments$fn_mzml_wpath)
    print(arguments$fn_corr_output)
    print(arguments$fn_ms2_output)
    print(arguments$fn_FT_wpath)
    print("-----------------------------")

    AIF <- openMSfile(arguments$fn_mzml_wpath)
    ...
    printMS2header(arguments$fn_ms2_output, arguments$fn_mzml)
